...
class HedgeFundInfo(Base):
    ...
    water_line = Column(DOUBLE(asdecimal=False), nullable=False)  # 初始水位线
...
class FOFInfo(Base):
    ...
    initial_raised_fv = Column(DOUBLE(asdecimal=False), nullable=False)  # 初始募集面值
    initial_net_value = Column(DOUBLE(asdecimal=False), nullable=False)  # 期初总资产净值


class FOFScaleAlteration(Base):
    '''fof规模变动记录'''
...
class FOFInvestorPosition(Base):
    '''fof投资人持仓信息'''
